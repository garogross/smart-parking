...
export const getAllParking = handleFactory.getAll(
  populateOptions,
...
export const openBareer = catchAsync(async (req, res, next) => {
  if (
    !req.params.type ||
    !Object.values(historyActionTypes).includes(req.params.type)
  ) {
  ...
  const data = {
    plateNumber:
      globalState[isExit ? "lastExitPlateNumber" : "lastEntryPlateNumber"],
